...
class Feedback extends Component {
  ...
  async handleSubmit(event) {
    ...
    this.setState(() => ({ status: response.data.status, message_res: response.data.message }));
  }
          ...
          <Collapsible.Advanced>
            <div className="container-xl p-4">
                {this.state.status === "200" || this.state.status === 200 ? (
                  ...
                  <>{this.state.message_res ? (
                    <>{this.state.message_res}</>
                  ) : (
                    <>
                    {this.state.status != '' ? (<>Ошибка отправки, свяжитесь с Администратором платформы</>) : null}
                    </>
                  )}</>
                )}
